...
用途：
建立一个无限周期重复的单个 supercell，用于扫描双 TiO2 柱的几何参数，
并建立 -1、0、+1 级次的 FDTD library。

模型设定：
  工作波长：905 nm。
  入射光：从 SiO2 基底侧沿 +z 方向正入射的 x 偏振平面波。
  基底：SiO2，初始折射率 n = 1.45。
  柱：TiO2，初始折射率 n = 2.30，柱高 650 nm。
  上包层：空气。
  x/y 边界：Periodic。
  z 边界：PML。
...
注意：
  当前柱半径、位置和柱高只是第一组可运行的起点，并非最终优化结构。
  扫描时应至少改变 r1、r2、x1、x2、h，且始终保持两个柱在 supercell 内不重叠。
